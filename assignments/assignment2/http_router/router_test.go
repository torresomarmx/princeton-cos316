...
func TestBasicGet(t *testing.T) {
	...
	request := httptest.NewRequest(httpGet, "/index.html", nil)
...
func TestBasicPaths(t *testing.T) {
	...
	for _, path := range paths {
		request := httptest.NewRequest(httpGet, path, nil)
		recorder := httptest.NewRecorder()
		router.ServeHTTP(recorder, request)
		response := recorder.Result()
...
func TestCapturePaths(t *testing.T) {
	...
	paths := []string{
		...
		"/users/omar/files",
		"/users/all",
		"/users/omar/pictures/1",
	}

	router := NewRouter()

	for _, route := range routes {
		router.AddRoute(httpGet, route, echoPathHandler)
	}
